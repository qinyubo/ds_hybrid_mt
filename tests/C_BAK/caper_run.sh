...
mpirun -machinefile host_server -n $NUM_WRITER $DIR/test_writer DATASPACES $NUM_WRITER 2 $NUM_WRITER 1 $(($CONF_DIMS/$NUM_WRITER)) $CONF_DIMS 3 1  &
mpirun -machinefile host_server -n $NUM_READER $DIR/test_reader DATASPACES $NUM_READER 2 $NUM_READER 1 $(($CONF_DIMS/$NUM_READER)) $CONF_DIMS 3 2  &
